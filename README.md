...
- ![GitHub Repo stars](https://img.shields.io/github/stars/theDoc001/fivedrisk?style=social) [**fivedrisk**](https://github.com/theDoc001/fivedrisk): Per-action runtime governance for AI agents. Deterministic 5-dimension scoring (Data, Tool, Reversibility, External, Autonomy), Markov SafetyDrift across compositional risk sequences, injection (24 patterns) and output-leakage scanners, append-only SQLite audit log plus NDJSON event stream for SIEM. `@gate` decorator, Claude Agent SDK hooks, LangGraph node. Apache-2.0, 424 tests, ~40µs p50 gate path.
